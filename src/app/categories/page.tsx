      ...
      <div className="container mx-auto px-4 pt-32 pb-24">
        <header className="mb-12 sm:mb-16 text-center">
          <motion.div
            ...
            className="inline-flex p-3 bg-amber-500/10 text-amber-500 rounded-2xl mb-4 sm:mb-6"
          >
          ...
          {categories.map((cat, i) => (
            <Link key={cat.id} href={`/products?category=${encodeURIComponent(cat.name)}`}>
              <motion.div
                ...
                className="group relative h-56 sm:h-[400px] rounded-2xl sm:rounded-[40px] overflow-hidden shadow-2xl border border-white/5 cursor-pointer"
              >
                <Image
                  src={cat.image}
                  alt={cat.name}
